<?php
require './../controllers/passwordForgot.php';
?>
        ...
        <div class="col-lg-4 col-md-8 col-sm-10 m-auto">
            <div id="card-login" class="w-100 card shadow-default ">
                <div id="title-login" class="card-title w-100 items-in-row py-3">
                    <label class="w-100 text-center m-auto text-primary font-18 text-bold">Restablecer Contraseña</label>
                </div>
                <?php if (!empty($result)) : ?>
                    <label class="text-info text-bold text-center font-18 w-100 pt-3 m-0"><i class="fas fa-exclamation-triangle mx-2"></i><?= $result ?></label>
                <?php endif; ?>
                <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
                    <p class="text-justify text-black-50 text-bold font-12 mx-3 pt-2 pb-0">Por favor, ingresa el correo asociado a tu cuenta, enviaremos un enlace con una contraseña al azar, puedes cambiarla cuando ingreses al sitio de nuevo.</p>
                    <p class="text-justify text-black-50 text-bold font-12 mx-3 pt-2 pb-0">Recomendamos cerrar sesión si aún te encuentras dentro del sitio.</p>
                    <div class="card-body row m-0 pt-0">
                        <label for="lblUsuario" class="text-black-50 text-bold font-18"><i class="fas fa-envelope mx-2 fh-2"></i>Correo</label>
                        <input id="inpMail" name="userEmail" class="form-component w-100 mb-4" type="email" required>
                    </div>
                    <div class="card-footer w-100 items-in-row py-2">
                        <button type="submit" value="Submit" class="button-primary text-white text-bold mx-auto">Restablecer<i class="fas fa-arrow-right ml-2"></i></button>
                    </div>
                </form>
                <a href="./Login.php" class="btn btn-outline-secondary font-weight-bold m-2" role="button"><i class="fas fa-arrow-left mx-2"></i>Regresar</a>
            </div>
        </div>
